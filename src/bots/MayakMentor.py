...
from src.rag.chain import get_mayakovsky_response_mentor
...
# ==================== КОНФИГУРАЦИЯ ====================
TOKEN = os.getenv("MAYAKMENTOR_TOKEN")
if not TOKEN:
...
async def cmd_start(message: Message):
    welcome_text = (
        ...
        "Вышибаю лень, громлю прокрастинацию.\n\n"
        "Пиши, товарищ, на что жалуешься? Лень работать? "
        "Рассказывай, а я отвечу так, что захочется горы свернуть!"
    )
...
async def handle_user_complaint(message: Message):
    ...
    try:
        bot_answer = get_mayakovsky_response_mentor(message.text)
